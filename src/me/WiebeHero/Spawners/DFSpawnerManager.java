...
public class DFSpawnerManager {
	...
	public void saveSpawnersBackup(String folder) {
		File f =  new File("plugins/CustomEnchantments/Data-Backups/" + folder + "/spawnerConfig.yml");
		YamlConfiguration yml = YamlConfiguration.loadConfiguration(f);
		try{
			yml.load(f);
        }
	...
	public void spawnMob(DFSpawner spawner) {
		...
		mob.getAttribute(Attribute.GENERIC_FOLLOW_RANGE).setBaseValue(9.5 + 1.5 * tier);
